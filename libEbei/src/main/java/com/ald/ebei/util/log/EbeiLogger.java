 ...
 * 修订历史：
 */
public final class EbeiLogger {
    private static final String TAG = "EbeiLogger";
    ...
    static {
        ...
        SAVE_PATH = EbeiDataUtils.getSDCardAppPath();
    ...
    private static void writeLog(int type, String msg) {
        ...
        // 如果没有读写SD卡的权限，则不写入文件
        if (null != context && !EbeiPermissionCheck.getInstance().checkPermission(context, Manifest.permission.WRITE_EXTERNAL_STORAGE)) {
            return;
        }
        ...
        try {
            ...
            msg = new StringBuilder()
                    .append("\r\n")
                    .append(getDateFormat(EbeiDateFormatter.SS.getValue()))
            ...
            final String fileName = new StringBuffer()
                    .append(LOG_PREFIX)
                    .append(getDateFormat(EbeiDateFormatter.DD.getValue()))
                    .append(".log").toString();

            recordLog(LOG_DIR, fileName, msg, true);
        } catch (Exception e) {
            EbeiLogger.e("EbeiLogger: ", e.getMessage());
        }
    }
    ...
    private static void write(final File file, final String msg, final boolean append) {
        new EbeiSafeAsyncTask<Void>() { // TODO: ...
            @Override
            public Void call() throws Exception {
                ...
                try {
                    ...
                    try {
                        ...
                        EbeiLogger.e(TAG, "write fail!!!", e);
                    } finally {
                        if (fos != null) {
                            try {
                                fos.close();
                            } catch (IOException e) {
                                EbeiLogger.d(TAG, "Exception closing stream: ", e);
                            }
                        }
                    }
